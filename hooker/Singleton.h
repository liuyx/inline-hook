...
class Singleton {
public:
	static typename ThreadingModel<T>::InstanceType& getInstance();
	void destroy() {
...
typename ThreadingModel<T>::InstanceType& Singleton<T,ThreadingModel,CreatePolicy, DestroyPolicy>::getInstance() {
	if (!pInstance_) {
